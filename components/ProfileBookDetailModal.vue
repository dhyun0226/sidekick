      ...
      <div class="relative w-full max-w-lg bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl border border-zinc-200 dark:border-zinc-800 my-8 overflow-hidden text-left">
        ...
        <div class="bg-gradient-to-b from-zinc-50 to-white dark:from-zinc-800 dark:to-zinc-900 border-b border-zinc-200 dark:border-zinc-800">
          <div class="flex items-center justify-between px-4 pt-4 pb-3">
            <h3 class="text-xs font-bold text-zinc-500 uppercase tracking-widest">Book Details</h3>
        ...
        <div class="border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900">
          <div class="flex px-4">
            <button
              v-for="t in (['all', 'review', 'comments'] as const)"
              :key="t"
              @click="tab = t"
              class="flex-1 py-3 text-sm font-bold transition-colors relative"
              :class="tab === t ? 'text-zinc-900 dark:text-white' : 'text-zinc-500'"
            >
              {{ t.charAt(0).toUpperCase() + t.slice(1) }}
              <div v-if="tab === t" class="absolute bottom-0 left-0 right-0 h-0.5 bg-lime-400"></div>
            </button>
          </div>
        </div>
          ...
          <template v-else>
            ...
            <div v-if="tab !== 'review'" class="space-y-4">
              <div
                ...
                class="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-4 transition-all cursor-pointer hover:border-lime-400"
              >
                <!-- Meta Info -->
...
const fetchBookActivities = async () => {
  ...
  try {
    ...
    const [ { data: comments }, { data: reviews } ] = await Promise.all([
      client.from('comments').select('id, content, anchor_text, position_pct, created_at, parent_id, parent:parent_id (content, anchor_text, user:users (nickname)), group_book:group_books (id, group:groups (name, id))').eq('user_id', user.id).eq('group_book_id', props.book.groupBookId).order('created_at', { ascending: false }),
      client.from('reviews').select('id, content, rating, created_at, group_book_id, group_book:group_books (id, group:groups (name, id))').eq('user_id', user.id).eq('group_book_id', props.book.groupBookId).order('created_at', { ascending: false })
    ])
