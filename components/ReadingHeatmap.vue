  ...
  <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-5">
    ...
    <div v-if="viewMode === 'month'" class="grid grid-cols-7 gap-1 mb-4">
      ...
      >
        <div v-if="day.activities?.length > 0" class="absolute inset-0 z-0 text-left">
          ...
          <div v-else class="w-full h-full bg-lime-50/30 dark:bg-lime-900/30">
        ...
        <span v-if="day.date" class="text-[11px] z-10" :class="{'text-zinc-400 dark:text-zinc-400': !day.isCurrentMonth, 'text-zinc-700 dark:text-zinc-300': day.isCurrentMonth && day.count === 0, 'text-white dark:text-white font-black drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)]': day.count > 0}">{{ day.dayNumber }}</span>
    ...
    <div class="pt-5 mt-2 border-t border-zinc-100 dark:border-zinc-800/50 grid grid-cols-3 gap-4">
      ...
      <div class="text-left border-l border-zinc-100 dark:border-zinc-800/50 pl-4">
        <p class="text-[11px] font-black text-zinc-400 dark:text-zinc-300 uppercase tracking-tighter mb-1">{{ statsLabel.activities }}</p>
        <p class="text-lg font-black text-zinc-900 dark:text-white leading-none">{{ currentStats.activities }}회</p>
      </div>
      <div class="text-left border-l border-zinc-100 dark:border-zinc-800/50 pl-4">
        <p class="text-[11px] font-black text-zinc-400 dark:text-zinc-300 uppercase tracking-tighter mb-1">최장 연속</p>
