<template>
  <div class="space-y-8 pb-10">
    ...
    <div v-else-if="timeline.length === 0 && !loading" class="py-12 flex flex-col items-center text-center">
      ...
      <p class="text-xs text-zinc-500 dark:text-zinc-400 mb-4">책을 읽으며 생각을 남겨보세요.</p>
      <button @click="router.push('/')" class="px-4 py-2 bg-zinc-900 dark:bg-white text-white dark:text-black text-xs font-bold rounded-lg">내 그룹으로 가기</button>
    </div>
    ...
    <div v-else class="space-y-8">
      <div v-for="monthGroup in timelineByMonth" :key="monthGroup.month" class="space-y-4">
        ...
        <div class="flex items-center gap-3 sticky top-[49px] z-20 bg-gray-50/95 dark:bg-[#09090b]/95 py-2 backdrop-blur-sm">
          <span class="text-xs font-black text-zinc-400 dark:text-zinc-500 uppercase">{{ monthGroup.month.replace('.', '년 ') }}월</span>
          ...
          >
            ...
            <div class="flex items-start gap-3 mb-3">
              ...
              <div class="min-w-0 flex-1 text-left">
                <h4 class="text-sm font-bold text-zinc-900 dark:text-white truncate mb-0.5 tracking-tight">{{ item.bookTitle }}</h4>
            ...
            <div v-if="item.isReply && item.parentData" class="mb-3 overflow-hidden rounded-lg border border-zinc-100 dark:border-zinc-800 text-left">
              <div class="bg-zinc-50 dark:bg-zinc-800/50 px-3.5 py-3">
                ...
                <div v-if="item.parentData.anchor_text" class="mb-2 pl-2 border-l-2 border-zinc-300 dark:border-zinc-600">
                  <p class="text-[13px] text-zinc-500 dark:text-zinc-400 italic font-serif leading-relaxed">{{ item.parentData.anchor_text }}</p>
                </div>
            ...
            <div v-if="item.anchor_text" class="mb-3 pl-3 border-l-2 border-zinc-200 dark:border-zinc-700 text-left">
              <p class="text-xs text-zinc-500 dark:text-zinc-400 italic leading-relaxed font-serif">{{ item.anchor_text }}</p>
            </div>
