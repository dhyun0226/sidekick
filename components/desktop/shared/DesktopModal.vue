      ...
      <div v-if="isOpen" class="fixed inset-0 z-[9999] flex items-center justify-center" @click.self="$emit('close')" @keydown.esc="$emit('close')" tabindex="-1">
        ...
        >
          ...
          <div v-if="title" class="flex items-center justify-between px-6 py-4 border-b border-zinc-100/80 dark:border-zinc-800/60">
            ...
            <button
              @click="$emit('close')"
              class="w-7 h-7 rounded-full flex items-center justify-center text-zinc-400 dark:text-zinc-300 hover:text-zinc-600 dark:text-zinc-400 dark:hover:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors"
            >
              <X :size="15" />
            </button>
          </div>
