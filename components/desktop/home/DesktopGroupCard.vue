  ...
  >
    <div class="flex items-center gap-4 p-5">
      ...
      <div v-else class="w-12 h-12 rounded-xl bg-zinc-50 dark:bg-zinc-800/50 flex items-center justify-center flex-shrink-0">
        <Coffee :size="18" class="text-zinc-400 dark:text-zinc-400" />
      </div>
      ...
      <div class="flex-1 min-w-0">
        <div class="flex items-center gap-2 mb-1">
          ...
          <span class="text-desktop-footnote text-zinc-400 dark:text-zinc-400 flex-shrink-0 flex items-center gap-0.5">
            <Users :size="10" />
            {{ group.memberCount }}
          </span>
        </div>
        <div class="flex items-center gap-3">
          <p v-if="group.currentBook" class="text-desktop-caption text-zinc-400 dark:text-zinc-400 truncate">
            {{ group.currentBook.title }}
          </p>
          <p v-else class="text-desktop-caption text-zinc-400 dark:text-zinc-400">
            새 책을 기다리고 있어요
          </p>
        </div>
      </div>
      ...
      <ChevronRight v-else :size="16" class="text-zinc-200 dark:text-zinc-400 flex-shrink-0" />
    </div>
  </div>
</template>
