    ...
    <div class="flex items-center gap-4 p-5">
      ...
      <div class="flex-1 min-w-0">
        ...
        <div class="flex items-center gap-3">
          <p v-if="group.currentBook" class="text-desktop-caption text-zinc-400 dark:text-zinc-300 truncate">
            {{ group.currentBook.title }}
          </p>
          <p v-else class="text-desktop-caption text-zinc-400 dark:text-zinc-300">
            새 책을 기다리고 있어요
          </p>
        </div>
      </div>
      ...
      <ChevronRight v-else :size="16" class="text-zinc-200 dark:text-zinc-300 flex-shrink-0" />
    </div>
  </div>
</template>
