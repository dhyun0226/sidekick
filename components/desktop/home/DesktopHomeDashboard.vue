    ...
    <template v-else>
      ...
      <section v-if="soloGroup" class="mb-10">
        ...
        <div v-else class="py-10 text-center">
          <p class="text-desktop-body text-zinc-400 dark:text-zinc-300 mb-5">읽을 책을 추가해보세요</p>
          <button
            @click="router.push('/my-library')"
      ...
      <div v-if="!soloGroup && readingGroups.length === 0 && idleGroups.length === 0" class="text-center py-24">
        <div class="w-16 h-16 mx-auto mb-6 rounded-2xl bg-zinc-50 dark:bg-zinc-800/50 flex items-center justify-center">
          <UsersIcon :size="24" class="text-zinc-400 dark:text-zinc-300" />
        </div>
        <h3 class="text-desktop-body font-semibold tracking-tight text-zinc-900 dark:text-white mb-2">공유 그룹에 참여해보세요</h3>
        <p class="text-desktop-callout text-zinc-400 dark:text-zinc-300 font-light mb-8">새로운 모임을 만들거나 초대를 받아보세요.</p>
        <div class="flex gap-3 justify-center">
          ...
          <button
            ...
            class="px-6 py-2.5 text-zinc-600 dark:text-zinc-300 font-medium rounded-full hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors duration-200 ease-apple text-desktop-callout"
          >
            초대 코드 입력
          </button>
        </div>
