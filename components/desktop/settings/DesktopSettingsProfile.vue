  ...
  <div class="pt-6 border-t border-zinc-200/80 dark:border-zinc-800">
    ...
    <div class="flex items-center gap-5 mb-6">
      <div class="w-16 h-16 rounded-full bg-zinc-100 dark:bg-zinc-800 overflow-hidden flex-shrink-0">
        ...
        <div v-else class="w-full h-full flex items-center justify-center text-desktop-body text-zinc-400 dark:text-zinc-300">
          {{ (profile?.nickname || 'U')[0] }}
        </div>
      </div>
      <div>
        <label class="px-3.5 py-1.5 text-desktop-callout text-zinc-600 dark:text-zinc-300 cursor-pointer ring-1 ring-zinc-200 dark:ring-zinc-700 rounded-full hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors ease-apple duration-200">
          사진 변경
    ...
    <div class="space-y-4">
      <div>
        <label class="text-desktop-caption text-zinc-400 dark:text-zinc-300 mb-1.5 block font-medium">닉네임</label>
        <input
          v-model="nickname"
