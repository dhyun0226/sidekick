  ...
  <div class="space-y-3">
    ...
    <div class="bg-zinc-50 dark:bg-zinc-900 rounded-xl ring-1 ring-black/[0.04] dark:ring-white/[0.06] p-4 border border-zinc-200/50 dark:border-zinc-800 select-none">
      <div class="flex justify-between items-center mb-3">
        <h4 class="text-xs font-bold text-zinc-500 dark:text-zinc-400">{{ new Date().getFullYear() }} 독서 목표</h4>
        <button
          v-if="!editingGoal"
          @click="$emit('start-edit-goal')"
          class="text-xs font-bold text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white active:scale-95 transition-all"
        >
          수정
        </button>
      </div>
      ...
      <div v-if="editingGoal" class="flex gap-2 mb-1">
        ...
        />
        <button @click="$emit('save-goal')" class="px-3 py-1.5 bg-zinc-900 dark:bg-white text-white dark:text-black rounded-lg text-xs font-bold">저장</button>
        <button @click="$emit('cancel-edit-goal')" class="px-3 py-1.5 bg-zinc-200 dark:bg-zinc-800 text-zinc-500 rounded-lg text-xs font-bold">취소</button>
      </div>
