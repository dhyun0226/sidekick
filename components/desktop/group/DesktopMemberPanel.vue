  ...
  <div class="space-y-5">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-desktop-footnote text-zinc-400 dark:text-zinc-400 mt-1">현재 책 기준 진행률입니다.</p>
      </div>
      <button
    ...
    <div class="space-y-0.5">
      ...
      >
        <div class="w-5 text-center flex-shrink-0 tabular-nums">
          <span class="text-desktop-caption" :class="index === 0 ? 'font-bold text-zinc-900 dark:text-white' : 'text-zinc-400 dark:text-zinc-300'">{{ index + 1 }}</span>
        </div>

        <div class="w-7 h-7 rounded-full overflow-hidden flex-shrink-0 bg-zinc-200 dark:bg-zinc-700">
          <img v-if="member.avatar_url" :src="member.avatar_url" class="w-full h-full object-cover" />
          <div v-else class="w-full h-full flex items-center justify-center text-zinc-500 dark:text-zinc-300 text-desktop-caption font-medium">
            {{ (member.nickname || 'U')[0] }}
          </div>
        </div>
