      ...
      <div v-if="books.length > 1" class="relative">
        ...
        >
          <button
            ...
            class="w-full px-3 py-2 text-left text-desktop-caption text-zinc-600 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors flex items-center justify-between"
          >
    ...
    <div v-if="sortedBooks.length > 0" class="space-y-0.5">
      ...
      >
        ...
        <div class="flex-1 min-w-0 py-0.5">
          ...
          <p class="text-desktop-footnote text-zinc-400 truncate mt-0.5">{{ book.author }}</p>
          <div class="flex items-center gap-2 mt-1.5 flex-wrap">
            ...
            <span v-if="book.genre" class="flex items-center gap-1">
              ...
              <span class="text-desktop-footnote text-zinc-400">{{ book.genre }}</span>
            </span>
            <!-- Rating -->
            <span v-if="book.averageRating" class="flex items-center gap-0.5 text-desktop-footnote text-zinc-400">
              <Star :size="10" fill="currentColor" class="text-amber-400" />
              {{ Number(book.averageRating).toFixed(1).replace('.0', '') }}
            </span>
            <!-- Review count -->
            <span v-if="book.reviewCount > 0" class="text-desktop-footnote text-zinc-400">
