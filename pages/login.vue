      ...
      <div class="w-full max-w-sm">
        ...
        <div class="grid grid-cols-3 gap-2.5 mb-8">
          <div
            ...
            :key="feat.title"
            class="bg-white dark:bg-zinc-900 rounded-xl p-3 ring-1 ring-black/[0.04] dark:ring-white/[0.06] text-center"
          >
        ...
        <div class="space-y-3">
          ...
          <button
            v-if="showSampleLogin"
            type="button"
            @click="signInWithSample"
            :disabled="Boolean(loading)"
            class="w-full py-3.5 px-6 bg-lime-400 hover:bg-lime-300 text-black rounded-2xl font-bold transition-all active:scale-[0.99] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
            <TestTube2 :size="18" />
            <span v-if="loading === 'sample'">샘플 계정 준비 중...</span>
            <span v-else>개발용 샘플 계정으로 로그인</span>
          </button>
        </div>
      </div>
    </div>
...
import { CalendarDays, MessageCircle, TestTube2, Users } from 'lucide-vue-next'
...
const loading = ref<false | 'google' | 'sample'>(false)

const showSampleLogin = computed(() => {
  return /^http:\/\/(localhost|127\.0\.0\.1)(:\d+)?/.test(config.public.baseUrl)
})
...
const signInWithSample = async () => {
  loading.value = 'sample'
  try {
    const sample = await $fetch<{ email: string; password: string }>('/api/dev/sample-user', {
      method: 'POST'
    })
    ...
    toast.error(error.data?.message || error.message || '샘플 로그인 중 오류가 발생했습니다.')
    loading.value = false
  }
}
</script>
