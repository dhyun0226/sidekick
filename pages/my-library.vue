...
import { Menu, Search, Plus, Settings, Share2, ChevronLeft, ChevronRight, ChevronDown, LogOut, MoreVertical, UserCheck, UserX, Edit2, Send, X, BarChart3, Copy, User, Archive, Pause } from 'lucide-vue-next'
...
const fetchSoloGroupId = async () => {
  ...
  if (error || !data) {
    console.error('[MyLibrary] Failed to fetch solo group:', error)
    return null
  }
...
// 내 서재는 본인만 사용하므로 항상 admin
const isAdmin = computed(() => true)

// 내 서재는 항상 solo
const isSoloGroup = computed(() => true)

// 내 서재는 절대 paused 상태가 아님
const isPausedGroup = computed(() => false)
const commentCount = computed(() => comments.value.length)

// Sorted members with progress
// 내 서재는 멤버가 본인만 있으므로 레이스 불필요
...
const fetchData = async () => {
  ...
  try {
    // 1. Solo 그룹 ID 먼저 가져오기
    const soloGroupId = await fetchSoloGroupId()
    if (!soloGroupId) {
      console.error('[MyLibrary] Solo group not found')
      toast.error('내 서재를 찾을 수 없습니다.')
      router.push('/')
      return
    }
    groupId.value = soloGroupId
    ...
    if (groupError || !groupData) {
      console.log('[MyLibrary] Group not found:', groupId.value)
      toast.error('내 서재를 찾을 수 없습니다.')
      router.push('/')
      return
    }
    ...
    // 내 서재는 멤버가 본인만 있으므로 members 조회 생략
  ...
  try {
    ...
    if (isSoloGroup.value) {
      ...
      if (statusError) {
        ...
        console.log('[UnmarkFinished] Book status changed back to reading')
      }
    }

    // 3. 데이터 새로고침 (DB에서 최신 데이터 가져오기)
    await loadMemberProgress(bookId)
    await fetchBooks()
...
const markAsCompleted = async () => {
  ...
  try {
    ...
    modals.markCompleted = false
    modals.editingBook = null
    toast.success('완독 처리되었습니다! 🎉 책장으로 이동합니다.')
