 ...
 *
 * 변경 방법:
 * UPDATE subscription_limits SET max_books_per_group = 5 WHERE tier = 'free';
 * UPDATE subscription_limits SET max_groups_created = 2 WHERE tier = 'free';
 */

-- ============================================
-- 1. 그룹 참가 제한 검사 함수 (DB 기반)
-- ============================================
...
-- 📝 현재 설정 확인:
SELECT * FROM public.subscription_limits;
*/
...
SELECT 'can_add_book_to_group 함수' as check_item,
       ...
       THEN '✅ 업데이트됨 (DB 기반)' ELSE '❌ 실패' END as status
UNION ALL
...
SELECT 'subscription_limits 데이터' as check_item,
       ...
       THEN '✅ 설정됨' ELSE '❌ 실패' END as status;

-- 현재 제한 설정 출력
SELECT '=== 현재 구독 제한 설정 ===' as info;
SELECT tier, max_groups_created as "최대 그룹", max_books_per_group as "그룹당 최대 책", has_statistics_access as "통계 접근"
FROM public.subscription_limits
ORDER BY
  CASE tier
    WHEN 'free' THEN 1
